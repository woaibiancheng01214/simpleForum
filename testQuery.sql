...
SELECT * FROM
( SELECT topic.forumId, forum.title as forumtitle, topic.topicId, topic.title as topictitle, post.postId
...
ON b.forumId = a.forumId AND b.postId = a.latest GROUP BY a.forumId;
...
-- AdvancedPersonView
  ...
  LEFT JOIN
  ( SELECT Topic.topicId AS topicId, MAX(postId) AS latest FROM Topic
  LEFT JOIN Post ON Topic.topicId = Post.topicId
  ...
  WHERE (Post.postId = a.latest OR Topic.topicId IS NULL) AND Person.username = 'ab12345'
  ...
  LEFT JOIN Person creator ON creator.id = creatorId
  LEFT JOIN
  ( SELECT Topic.topicId as topicId, MAX(postId) AS latest FROM Forum
  LEFT JOIN Topic ON Topic.forumId = Forum.id
  LEFT JOIN Post ON Topic.topicId = Post.topicId
  ...
  WHERE (Post.postId = a.latest OR Topic.topicId IS NULL) AND Forum.id=1 ORDER BY forumTitle;
